...
import { MoreVertical, Share2, Pencil, Trash2, Download } from 'lucide-react';
import { getFileIcon, FolderIcon, formatDate, formatBytes, triggerFileDownload } from '../utils/fileUtils';
  ...
  const ctxItems = ctx
    ? [
        !ctx.isFolder && {
          ...
          action: async () => {
            try {
              const res = await folderAPI.getFileUrl(ctx.item.id);
              triggerFileDownload(res.data, ctx.item.name);
            } catch {
              // optional fallback
            }
          },
